...
wrapped the model in a loop, the \emph{agent}, in which the model
reasons about a goal, invokes external tools, observes their results, and
iterates until the task is done~\cite{yao2023react}. The current generation
...
code analysis, data-processing swarms) instead run on
...
They retry, hallucinate tool arguments, hang on remote calls, and
...
the swarm doing right now? Which agents are talking to which? Why is this
...
agent-to-agent call (caller, callee, correlation, outcome), a context-memory
operation (a node added to or evicted from an agent's working context), a
...
kreps2013log}. This thesis builds on ChronoLog~\cite{kougkas2020chronolog}, a
distributed, tiered shared-log store that orders events by physical time
and, crucially for this setting, places a log-capture process (a
\emph{keeper}) on every cluster node. Agents write locally on the node
where they run; the log provides global ordering, durability, and tiered
archival; and every observability view is a \emph{reader} over the same
substrate. Ordering and durability are properties of the storage layer, not
of the instrumentation, and cross-agent causal reconstruction comes with the
substrate rather than being bolted on.
...
Two capture paths feed the log, one for LLM turns and one for inter-agent
...
drain latency, and a live replay can block a reader for minutes), the
...
views from the same event stream at ingest time. We validate the system
...
\item \textbf{Fleet-scale analytics computed on the log.} Three analyses turn
  the captured events into operator answers: ChronoDoctor, which detects
  failures, clusters them by signature into ranked incidents, and attaches
  zero-cost diagnoses with cross-run incident memory; fleet-health rollups
...
requirements. The system design follows in Chapter~\ref{chap:design}: the
capture paths, the mapping of agent semantics onto the log, delivery
semantics, and the hot/cold read split. Chapter~\ref{chap:analytics} builds
