...
Chapter~\ref{chap:design} produced a complete event record: every observable
event of the fleet, durably ordered and attributed to node and agent,
readable either from the log itself once it has drained (the cold path,
complete but tens of seconds late) or from the ingest-time hot store that
serves the interval before that (\S\ref{sec:design-read}).
This chapter presents the analyses that turn that record into
...
The first operator question is situational awareness: what is the fleet doing,
right now, and where. The system answers it with two live graphs, built as
\emph{two projections of the same event stream}.

Both are built from the inter-agent plane, Path-B of
\S\ref{sec:design-overview}: every delegation between agents was captured as
a \texttt{start} and a \texttt{done} sharing a correlation identifier, and
reassembled into one \emph{stitched edge} carrying both endpoint sessions,
both hosts, the outcome, and the latency (\S\ref{sec:design-mapping}). Being
live, both graphs read the \emph{hot store} rather than the log: the
ingest-time copy that \S\ref{sec:design-read} introduced precisely because
the log itself cannot be read inside its visibility window.

The \emph{scenario graph} projects those edges onto agent identity: nodes
...
\emph{live bus}, the publish/subscribe fan-out of
\S\ref{sec:design-read} that pushes each newly ingested event to the
browser: the stored backlog first, then live per-event frames.
LLM-observability tools already offer this view
...
\section{Failure Detection, Clustering, and Diagnosis}
\label{sec:analytics-doctor}

The second is not ``show me the errors'' but ``tell me what
is wrong'': at fleet scale, a per-event error feed is least readable
when it matters most. This section describes the analysis built for that
question: a five-stage pipeline
(\emph{gather} $\to$ \emph{detect} $\to$ \emph{cluster} $\to$
\emph{diagnose} $\to$ \emph{record}) that turns raw events into a
ranked list of explained incidents. It is a component of the system
presented here, not an external tool; the dashboard exposes it as the
\emph{Doctor} tab, and the remainder of the thesis calls it the Doctor.
The shape is the one established by
...
lines; Oliner et al.~\cite{oliner2012loganalysis} survey the wider design
space that shape sits in. What differs here is the input, which is not free-text logs but a
structured, causally ordered event stream, so the detectors can key on
typed fields rather than on parsed text.

\paragraph{Gather and detect.} Gathering is the only I/O, and it reads only
hot sources: interactions and recoveries from the Path-A spool, stitched
edges from the Path-B hot store, with explicit bounds on sessions and
scenarios scanned. Neither read touches the log, which matters because a
scan runs inside the dashboard process: a replay that landed in the
visibility window would block that process outright (C2,
\S\ref{sec:design-gaps}), taking the whole interpreter with it.
...
\paragraph{Clustering.} The view scales because fleet-wide failures are
\emph{textually self-similar}. Each signal's
...
Drain~\cite{he2017drain}; the fixed variant is chosen here because
...
fallback catches the rest. Attributing a failure to a responsible component
is the problem Pinpoint~\cite{chen2002pinpoint} posed for internet
services, and it solved it by correlating the success or failure of many
requests with the components each one touched. The setting here is easier
in one specific respect: causal attribution is already explicit in the
events (\S\ref{sec:design-mapping}), so what remains is naming the cause,
not locating it.

The engine costs nothing per scan (R6). The learned alternative, sequence
models over log streams of which DeepLog~\cite{du2017deeplog} is the
canonical instance, generalizes to anomalies no rule anticipates, but buys
that with a training corpus and a per-event inference budget: exactly the
per-scan cost R6 forbids. An
...
\paragraph{Recording recurrence.} The final stage answers whether the
failure has happened before. Every scan
appends one line
...
The fourth question is population-level: which node is unlike the
...
The third question, why this scenario is slow, is the classical
...
span, the pivot to the fleet view is immediate: if the bottleneck's node also glows in
...
token counts, and costs behind the node), which is the join of
...
(Fig.~\ref{fig:ui-health}).} The Doctor surface. The center
...
(Fig.~\ref{fig:ui-cluster}).} This view puts the storage layer itself on
...
heatmap stays legible at hundreds, and the red cells name the offending
node before a single query is typed. Clicking a cell filters the fleet to that
...
edge's detail. The question of why the scenario is slow is answered by
naming the responsible hop, without requiring the operator to read the
trace.
