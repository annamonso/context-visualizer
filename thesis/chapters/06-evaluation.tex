...
(R2, \S\ref{sec:eval-freshness}); is PrismaDoctor's diagnosis
...
\bottomrule
\end{tabular}
\caption[Capture overhead, measured]{Capture overhead, measured on a live 8-node deployment
...
\bottomrule
\end{tabular}
\caption[Freshness and query latency, measured]{Freshness and query latency on the live 8-node deployment
...
\emph{correct}. The experiment scores a full PrismaDoctor scan (gather,
...
time and the corpus re-scanned, testing whether the incident history
...
\bottomrule
\end{tabular}
\caption[PrismaDoctor detection quality against ground truth]{PrismaDoctor detection quality against a ground-truth fault
...
absence (Q2). The fleet heatmap turns ``the fleet looks unhealthy'' into
``it is this machine'': one red cell among ok nodes (Q4). The trace
waterfall for the affected scenario charges the bottleneck hop by
self-time, landing on that same node (Q3). Clicking the offending agent
opens its conversation, the actual turns, retries and costs behind the
symptom, completing the pivot in two clicks (Q1). Re-running the fault
and re-scanning badges the incident \texttt{recurring $\times N$}: the
second occurrence is recognized, not rediscovered (Q5).
